...
class IndexRoute {
    ...
    async visao_geral(req, res) {
        ...
        rows = await (0, amazonbooks_1.executar)(`SELECT proScrapDate as date, proReview, proName From Product WHERE catCode = 1 and proPosition <= 5 and proName IN (Select proName FROM Product WHERE proPublisher != "N/A" and catCode = 1 GROUP BY proName ORDER by count(proName) DESC LIMIT 5) ORDER by proName, proScrapDate`);
        ...
        rows = await (0, amazonbooks_1.executar)(`SELECT c.catName, round(avg(proReview), 2) as avgReview FROM Product p INNER JOIN Category c ON c.catCode = p.catCode WHERE proReview != "N/A" GROUP BY c.catName ORDER BY avgReview DESC LIMIT 5`);
        rows.forEach((r) => {
    ...
    async autoajuda_2(req, res) {
        ...
        res.render("index/selfHelp/selfHelp2", {
            ...
            author_most_books: await (0, amazonbooks_1.executar)(`SELECT a.autName, COUNT(DISTINCT p.proName) AS countProName FROM Author a INNER JOIN Product p ON a.autCode = p.autCode WHERE p.catCode = 2 GROUP BY a.autName ORDER BY countProName DESC LIMIT 1;`),
            most_consistent_author: await (0, amazonbooks_1.executar)(`SELECT COUNT(p.autCode) AS countAutCode, a.autName FROM Product p INNER JOIN Author a ON p.autCode = a.autCode WHERE p.catCode = 2 GROUP BY p.autCode ORDER BY countAutCode DESC LIMIT 1;`),
            series_stars_page: JSON.stringify(series_stars_page)
        });
    }
    /* AUTOAJUDA - PG 3 */
    async autoajuda_3(req, res) {
        let rows;
        let hq_catTyp = { data: [] };
        let hq_catPriPag = {};
    ...
    async infantil_2(req, res) {
        ...
        res.render('index/kids/kids2', {
            ...
            author_most_books: await (0, amazonbooks_1.executar)(`SELECT a.autName, COUNT(DISTINCT p.proName) AS countProName FROM Author a INNER JOIN Product p ON a.autCode = p.autCode WHERE p.catCode = 13 GROUP BY a.autName ORDER BY countProName DESC LIMIT 1;`),
            most_consistent_author: await (0, amazonbooks_1.executar)(`SELECT COUNT(p.autCode) AS countAutCode, a.autName FROM Product p INNER JOIN Author a ON p.autCode = a.autCode WHERE p.catCode = 13 GROUP BY p.autCode ORDER BY countAutCode DESC LIMIT 1;`),
            series_stars_page: JSON.stringify(series_stars_page)
        });
    }
    /* INFANTIL - PG 3 */
    async infantil_3(req, res) {
        let rows;
        let hq_catTyp = { data: [] };
    ...
    async editoras(req, res) {
        ...
        /* TREEMAP */
        let treeType = [{ data: [] }];
        rows = await (0, amazonbooks_1.executar)(`SELECT proPublisher, count(proPublisher) as freq FROM Product WHERE proPublisher != "N/A" GROUP BY proPublisher ORDER BY freq DESC LIMIT 10;`);
        rows.forEach((r) => {
            treeType[0].data.push({
                x: r.proPublisher,
                y: r.freq
    ...
    /* EDITORAS - PG 2 */
    async editoras_2(req, res) {
        let rows;
        let pubFreq = [], pubRev = [];
        /* TOP */
        /* pub x freq */
        rows = await (0, amazonbooks_1.executar)(`Select proPublisher, count(proPublisher) as freq FROM Product p WHERE proPublisher != "N/A" GROUP BY proPublisher ORDER by freq DESC LIMIT 10`);
        rows.forEach((r) => {
        ...
        res.render("index/publishers/publishers2", {
            ...
            more_published: await (0, amazonbooks_1.executar)('SELECT proPublisher,  COUNT(DISTINCT proName) AS countProName FROM Product WHERE proPublisher != "N/A" AND proPublisher IS NOT NULL GROUP BY proPublisher ORDER BY countProName DESC LIMIT 1;')
        });
    }
    /* RELATÓRIO */
